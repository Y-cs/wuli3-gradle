...
[`事件发布说明`](event-publication.md)取代。公共 API、配置和模块行为仍以源码与发布版本为准。
...
- MySQL、Redis、Elasticsearch、MongoDB 模块当前是依赖聚合/占位 starter；RocketMQ starter 已提供事件传输适配。
