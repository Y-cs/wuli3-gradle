...
CLI 和 Gradle 任务均会在生成工程根目录创建 `AGENTS.md`，包含模块边界、构建风格、编码风格、测试指南、提交规范、Git 使用、注释和配置安全约束。
规范使用生成时的服务名、领域名和 Java 包名，随内置模板发布，不依赖运行目录中的 `AGENTS.md`，也不包含生成器仓库的本机路径或 OMX 配置。
