...
本文说明 web starter 提供的统一响应处理能力、使用方式、返回内容和未覆盖场景。
...
| 字段 | 说明 |
| --- | --- |
| `code` | 外部错误码。成功固定为 `0`；失败由 `ErrorCodeResolver` 解析。 |
...
| `IllegalArgumentException` | 400 | `WEB.BAD_REQUEST` | Controller 链路抛出的非法参数异常。 |
