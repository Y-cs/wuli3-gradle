...
/**
 * 将框架异常和业务异常映射为 {@link ApiResponse} 响应。
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleErrorCodeException(ErrorCodeException ex) {
        ...
        return buildResponse(status(ex.getResolvedErrorPolicy()
    ...
    public ResponseEntity<ApiResponse<Void>> handleMethodNotSupported(HttpRequestMethodNotSupportedException ex) {
        return buildResponse(HttpStatus.METHOD_NOT_ALLOWED, WebErrors.BAD_REQUEST, WebErrors.BAD_REQUEST.getMessage(),
                WebErrors.BAD_REQUEST);
    }

    @ExceptionHandler(HttpMediaTypeNotSupportedException.class)
    public ResponseEntity<ApiResponse<Void>> handleMediaTypeNotSupported(HttpMediaTypeNotSupportedException ex) {
        return buildResponse(HttpStatus.UNSUPPORTED_MEDIA_TYPE, WebErrors.BAD_REQUEST,
                WebErrors.BAD_REQUEST.getMessage(), WebErrors.BAD_REQUEST);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiResponse<Void>> handleException(Exception ex) {
        return buildResponse(HttpStatus.INTERNAL_SERVER_ERROR, WebErrors.INTERNAL_ERROR,
                WebErrors.INTERNAL_ERROR.getMessage(), WebErrors.INTERNAL_ERROR);
    }

    private ResponseEntity<ApiResponse<Void>> buildResponse(HttpStatus status, ErrorCode fallbackCode, String message,
    ...
    private static String visibleMessage(ErrorCodeException ex) {
        ErrorVisibility visibility = ex.getResolvedErrorPolicy()
                .visibility();
