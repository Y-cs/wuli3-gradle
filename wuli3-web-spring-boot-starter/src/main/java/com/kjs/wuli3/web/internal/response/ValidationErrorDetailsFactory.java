...
final class ValidationErrorDetailsFactory {
    ...
    @Nullable ValidationErrorDetails detail(final Exception ex) {
        ...
        return switch (ex) {
            ...
            case MissingServletRequestParameterException missingParameterException -> new ValidationErrorDetails(
                    List.of(new ValidationErrorDetails.Item(missingParameterException.getParameterName(),
                            ValidationErrorDetailsFactory.detailMessage(missingParameterException), null)));
            case MethodArgumentTypeMismatchException typeMismatchException -> new ValidationErrorDetails(
                    List.of(new ValidationErrorDetails.Item(typeMismatchException.getName(),
    ...
    private static ValidationErrorDetails methodArgumentNotValidDetail(final MethodArgumentNotValidException ex) {
        ...
        for (final FieldError fieldError : ex.getBindingResult()
                .getFieldErrors()) {
            errors.add(new ValidationErrorDetails.Item(fieldError.getField(),
                    ValidationErrorDetailsFactory.message(fieldError), fieldError.getRejectedValue()));
        }
        for (final ObjectError objectError : ex.getBindingResult()
                .getGlobalErrors()) {
            errors.add(new ValidationErrorDetails.Item(objectError.getObjectName(),
                    ValidationErrorDetailsFactory.message(objectError), null));
        }
    ...
    private static ValidationErrorDetails constraintViolationDetail(final ConstraintViolationException ex) {
        final List<ValidationErrorDetails.Item> errors = ex.getConstraintViolations()
                .stream()
                .map(violation -> new ValidationErrorDetails.Item(
                        ValidationErrorDetailsFactory.lastPathNode(violation.getPropertyPath()), violation.getMessage(),
