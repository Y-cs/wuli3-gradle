...
public class WebExceptionHandler {
    ...
    public WebExceptionHandler(final ApiResponseFactory responseFactory,
    ...
    public ResponseEntity<?> handleErrorCodeException(final ErrorCodeException ex, final HttpServletRequest request) {
        ...
        if (NativeResponseSupport.isAll(request)) {
            ...
            return WebErrorResponseMapper.nativeError(status, responseCode, message, this.requestId(), this.responseFactory);
        }
        return ResponseEntity.status(status)
                .body(this.responseFactory.fail(responseCode, message));
    }
    ...
    public ResponseEntity<?> handleNotFound(final Exception ex, final HttpServletRequest request) {
        return this.handleFrameworkException(ex, request, this.webErrorStatusResolver.resolve(ex, WebErrors.NOT_FOUND),
                WebErrors.NOT_FOUND, WebErrors.NOT_FOUND.getMessage(), null);
    }

    @ExceptionHandler(ResponseStatusException.class)
    public ResponseEntity<?> handleResponseStatusException(final ResponseStatusException ex,
            final HttpServletRequest request) {
        final HttpStatus status = this.webErrorStatusResolver.resolve(ex, WebErrorResponseMapper.responseCode(
                HttpStatus.valueOf(ex.getStatusCode()
                        .value())));
        final ErrorCode responseCode = WebErrorResponseMapper.responseCode(status);
        this.alert(ex, request, status, responseCode);
        if (NativeResponseSupport.isAll(request)) {
            return WebErrorResponseMapper.nativeProblemDetail(status.value(), WebErrorResponseMapper.message(ex), this.requestId());
        }
        return ResponseEntity.status(status)
    ...
    public ResponseEntity<?> handleErrorResponseException(final ErrorResponseException ex,
            final HttpServletRequest request) {
        final HttpStatus status = this.webErrorStatusResolver.resolve(ex, WebErrorResponseMapper.responseCode(
                HttpStatus.valueOf(ex.getStatusCode()
                        .value())));
    ...
    public ResponseEntity<?> handleMessageNotWritable(final HttpMessageNotWritableException ex,
            final HttpServletRequest request) {
        /*
         * 响应写出失败通常是服务端序列化或消息转换问题，不能向客户端暴露内部异常细节。
         */
        return this.handleFrameworkException(ex, request, HttpStatus.INTERNAL_SERVER_ERROR, WebErrors.INTERNAL_ERROR,
                WebErrors.INTERNAL_ERROR.getMessage(), null);
    }
    ...
    public ResponseEntity<?> handleException(final Exception ex, final HttpServletRequest request) {
        ...
        this.alert(ex, request, status, responseCode);
        if (NativeResponseSupport.isAll(request)) {
            return WebErrorResponseMapper.nativeProblemDetail(status.value(), responseCode.getMessage(), this.requestId());
        }
        return ResponseEntity.status(status)
                .body(this.responseFactory.fail(responseCode, responseCode.getMessage()));
    }
    ...
    private ResponseEntity<?> handleFrameworkException(final Exception ex, final HttpServletRequest request) {
        ...
        final ErrorCode responseCode = WebErrorResponseMapper.responseCode(status);
        final String message = responseCode == WebErrors.INTERNAL_ERROR ? WebErrors.INTERNAL_ERROR.getMessage()
                                                                        : responseCode.getMessage();
        return this.handleFrameworkException(ex, request, status, responseCode, message,
                this.validationErrorDetailsFactory.detail(ex));
    }

    private ResponseEntity<?> handleFrameworkException(final Throwable error, final HttpServletRequest request,
            final HttpStatus status, final ErrorCode responseCode, final String message,
        ...
        return ResponseEntity.status(status)
                .body(this.responseFactory.fail(responseCode, message, detail));
    }

    private void alert(final Throwable error, final HttpServletRequest request, final HttpStatus status,
            final ErrorCode responseCode) {
        this.errorAlertNotifiers.dispatch(new ErrorAlertContext(error, status, responseCode, this.requestId(),
